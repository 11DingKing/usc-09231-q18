...
class TestPointerPos(unittest.TestCase):
    ...
    def test_the_shape_is_composited_where_the_server_says(self) -> None:
        """--localcursor composites at capture time, at the server's
        position, and leaves the framebuffer itself bare."""
        handshake(self.cli, 400, 400)
        self.cli.factory.cursor = CursorMode.LOCAL
        ...
        self.assertEqual(self.cli.frame().getpixel((150, 120)), IMAGE_2X2[0])
