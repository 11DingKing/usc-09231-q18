...
# Decoders produce pixels in these formats regardless of what was negotiated;
# the rectangle handler says which one it produced.
RGB24 = PixelFormat(24, 24, False, True, 255, 255, 255, 0, 8, 16)
RGBX32 = PixelFormat(32, 24, False, True, 255, 255, 255, 0, 8, 16)

# Tight encoding, rfbproto §7.7.2
TIGHT_FILL = 0x08
TIGHT_JPEG = 0x09
...
class RFBClient(Protocol):  # type: ignore[misc]
    ...
    # Until ServerInit says otherwise the framebuffer is empty.
    width = 0
    height = 0
    ...
    def __init__(self) -> None:
        ...
        self._tight_streams = [zlib.decompressobj(0) for _ in range(4)]
        self._tight_length = 0
        self.rectangles = 0
    ...
    def _handleDecodeZRLEdata(
        ...
        for subencoding in it:
            ...
            if subencoding & 0x80:
                ...
                self.updateRectangle(tx, ty, tw, th, bytes(pixel_data), RGBX32)
            else:
                # No RLE
                if palette_size == 0:
                    # Raw pixel data
                    ...
                    self.updateRectangle(
                        tx, ty, tw, th, bytes(color) * pixels_in_tile, RGBX32
                    )
    ...
    # ---  Tight Encoding
    #
    # Only the parts a screenshot client meets in practice: fill and JPEG
    # rectangles, and basic compression with the copy filter.  The palette
    # and gradient filters are refused rather than decoded wrongly.
    def _tightBypp(self) -> int:
        pixel_format = self.pixel_format
        if pixel_format.truecolor and pixel_format.depth == 24 and pixel_format.bpp == 32:
            return 3
        return pixel_format.bypp

    def _tightPixelFormat(self) -> PixelFormat:
        if self._tightBypp() == 3:
            # A depth-24 pixel travels as its three colour bytes.
            pixel_format = self.pixel_format
            return PixelFormat(
                24,
                24,
                pixel_format.bigendian,
                True,
                255,
                255,
                255,
    ...
    def _handleDecodeTight(
        self, block: bytes, x: int, y: int, width: int, height: int
    ) -> None:
        control = block[0]
        for stream in range(4):
            if control & (1 << stream):
                self._tight_streams[stream] = zlib.decompressobj(0)
        kind = control >> 4
        if kind == TIGHT_FILL:
            self.expect(
                self._handleDecodeTightFill, self._tightBypp(), x, y, width, height
            )
        elif kind == TIGHT_JPEG:
            self._expectTightData(self._handleDecodeTightJpeg, x, y, width, height)
        elif kind & 0x08:
            self.vncProtocolError(f"unsupported Tight compression {kind:#x}")
            self.transport.loseConnection()
        else:
            stream_id = kind & 0x03
            if kind & TIGHT_EXPLICIT_FILTER:
                self.expect(
                    self._handleDecodeTightFilter, 1, x, y, width, height, stream_id
                )
            else:
                self._tightCopy(x, y, width, height, stream_id)

    def _handleDecodeTightFill(
        self, block: bytes, x: int, y: int, width: int, height: int
    ) -> None:
        self.updateRectangle(
            x, y, width, height, block * (width * height), self._tightPixelFormat()
        )
        self._doConnection()

    def _handleDecodeTightJpeg(
        self, block: bytes, x: int, y: int, width: int, height: int
    ) -> None:
        from PIL import Image  # Pillow is optional until an image is needed

        image = Image.open(io.BytesIO(block)).convert("RGB")
        self.updateRectangle(x, y, width, height, image.tobytes(), RGB24)
        self._doConnection()

    def _handleDecodeTightFilter(
        self, block: bytes, x: int, y: int, width: int, height: int, stream_id: int
    ) -> None:
        filter_id = block[0]
        if filter_id == TIGHT_FILTER_COPY:
            self._tightCopy(x, y, width, height, stream_id)
        else:
            self.vncProtocolError(f"unsupported Tight filter {filter_id:#x}")
            self.transport.loseConnection()

    def _tightCopy(self, x: int, y: int, width: int, height: int, stream_id: int) -> None:
        size = width * height * self._tightBypp()
        if size < TIGHT_MIN_TO_COMPRESS:
            self.expect(self._handleDecodeTightRaw, size, x, y, width, height)
        else:
            self._expectTightData(
                self._handleDecodeTightZlib, x, y, width, height, stream_id
            )

    def _handleDecodeTightRaw(
        self, block: bytes, x: int, y: int, width: int, height: int
    ) -> None:
        self.updateRectangle(x, y, width, height, block, self._tightPixelFormat())
        self._doConnection()

    def _handleDecodeTightZlib(
        self, block: bytes, x: int, y: int, width: int, height: int, stream_id: int
    ) -> None:
        data = self._tight_streams[stream_id].decompress(block)
        self.updateRectangle(x, y, width, height, data, self._tightPixelFormat())
        self._doConnection()

    def _expectTightData(
        self,
        ...
        *args: Any,
        ...
        """Read a compact-length, then hand ``length`` bytes to ``handler``."""
        self.expect(self._handleTightLength1, 1, handler, x, y, width, height, *args)

    def _handleTightLength1(
        self, block: bytes, handler: Callable[..., None], *args: Any
    ) -> None:
        byte = block[0]
        if not byte & 0x80:
            self.expect(handler, byte, *args)
        else:
            self._tight_length = byte & 0x7F
            self.expect(self._handleTightLength2, 1, handler, *args)

    def _handleTightLength2(
        self, block: bytes, handler: Callable[..., None], *args: Any
    ) -> None:
        byte = block[0]
        if not byte & 0x80:
            self.expect(handler, self._tight_length | (byte << 7), *args)
        else:
            self._tight_length |= (byte & 0x7F) << 7
            self.expect(self._handleTightLength3, 1, handler, *args)

    def _handleTightLength3(
        self, block: bytes, handler: Callable[..., None], *args: Any
    ) -> None:
        length = self._tight_length | (block[0] << 14)
        self.expect(handler, length, *args)
    ...
    def updateRectangle(
        ...
        :param data: bytes in the given pixel format.
        :param pixel_format: the format ``data`` is in, usually the
            negotiated one; decoders that convert say what they produced.
        """
